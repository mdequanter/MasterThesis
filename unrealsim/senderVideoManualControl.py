...
for arg in sys.argv[1:]:
    if arg.startswith("USE_VIDEO="):
        ...
        try:
            ...
            print("⚠️ Invalid MAX_FPS. Keeping:", MAX_FPS)
        ...
        try:
            ...
            print("⚠️ Invalid JPEG_QUALITY. Keeping:", JPEG_QUALITY)
    elif arg.startswith("WIDTH="):
        try:
            WIDTH = int(arg.split("=")[1])
        except ValueError:
            print("⚠️ Invalid WIDTH. Keeping:", WIDTH)
    elif arg.startswith("HEIGHT="):
        try:
            HEIGHT = int(arg.split("=")[1])
        except ValueError:
            print("⚠️ Invalid HEIGHT. Keeping:", HEIGHT)
...
def encrypt_data(plain_text):
    ...
    encryption_time = (encrypt_end_time - encrypt_start_time) * 1_000
...
async def send_messages(websocket):
    ...
    global FRAMELIMIT,should_exit,LATEST_POWER,POWERCPU,LAT,LON,CONNECTIONSTATUS
    if ANALYTICS:
    ...
    while not should_exit:
        ...
        if USE_VIDEO:
            ...
            if not ret or frame is None:
                print("Frame not available, skip")
                continue
        ...
        # Keyboard control: q, +, -, a, l
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            print("⏹️ Exit by user. Press Ctrl-x in terminal to close.")
            cv2.destroyAllWindows()
            should_exit = True
            break
        elif key in (ord('+'), ord('=')):  # Plus
            MAX_FPS += 1
            print(f"MAX_FPS -> {MAX_FPS}")
        elif key in (ord('-'), ord('_')):  # Min
        ...
        compression_time = (t1 - t0) * 1_000

        bitrate = displayFPS * size_kb * 8 / 1000  # Mbps
...
async def main():
    async with websockets.connect(SIGNALING_SERVER) as websocket:
        print(f"✅ Connected to {SIGNALING_SERVER}")
...
try:
    ...
    print("⏹️ Shutdown by KeyboardInterrupt")
finally:
    capture.release()
