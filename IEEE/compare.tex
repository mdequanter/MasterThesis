...
\begin{document}
\history{Date of publication Jan.~27,~2026; date of current version Jan.~27,~2026.}
\doi{To be added}
...
\corresp{Corresponding author: Dequanter M. Author (e-mail: maarten.dequanter@ehb.be)}


\begin{abstract}
Visually impaired persons (VIPs) face a lot of challenges in safe outdoor navigation, particularly in unstructured environments where conventional aids offer limited situational awareness. This paper evaluates the feasibility of real-time vision-based assistive navigation using semantic segmentation with edge and cloud-assisted artificial intelligence over public 4G/5G networks. A reproducible benchmarking framework based on prerecorded first-person video streams is introduced to enable controlled and safe experimentation. Low-power edge devices capture, compress, and encrypt video frames, which are offloaded to a remote GPU server for real-time inference. Experimental results show that cloud-assisted inference over public 5G networks consistently achieves end-to-end latencies below 200~ms at walking speed frame rates, while on-device inference on lightweight platforms remains insufficient. In addition, queue dynamics are shown to provide an early indicator of latency increase, enabling proactive fail-safe mechanisms. Power measurements confirm the energy efficiency of cloud offloading, supporting practical mobile deployment.
...
In this work, the RPI primarily acts as a frame capture and transmission unit. Camera images are captured, compressed, and encrypted on-device before being transmitted to the remote inference server, while returned guidance signals are rendered via visual, audio, or haptic feedback. A RP5 was used for end-to-end benchmarking over Wi-Fi and public 5G networks, while both RPI4 and RPI5 were evaluated for local on-device segmentation inference.
...
A limited dataset of 231 video frames was extracted from a prerecorded tour in the Unreal Engine “twin city” park environment and split into train/validation/test = 162(70\%)/47(20\%)/22(10\%). Images were resized to 640\,$\times$\,640 (average size \(\approx\)0.41\,MP). Annotation was performed in Roboflow (see Figure \ref{fig:trainingannotation}).
...
The segmentation model is trained to predict the walkable path in a park environment. On the validation set, it achieves a mask precision of 0.998 and a recall of 1.00, indicating that almost all walkable regions are correctly detected while virtually no non-walkable areas are incorrectly classified as path. The mean average precision at an Intersection over Union (IoU) threshold of 0.5 (mAP50) for the walkable-path masks reaches 0.995, while the more stringent mAP50–95 metric still yields 0.877. This shows that the model not only detects the presence of the path but also delineates its boundaries with high spatial accuracy, which is crucial for deriving a reliable center line for navigation. These results reflect in-domain validation performance, as both training and validation images originate from the same virtual park walk in the Unreal Engine environment. Consequently, within this validation domain, the risk of guiding a user towards non-walkable regions (false positives) is very low, and the available path width is largely preserved (few false negatives). However, these results may overestimate robustness to unseen park layouts, textures, or environmental conditions, motivating further out-of-domain evaluation.
In addition to the quantitative simulation-based validation, the model was qualitatively evaluated in an unseen real-world park environment. In these exploratory tests, the walkable path was consistently detected and a stable centerline suitable for navigation was produced. As no ground-truth annotations were available, these results are reported qualitatively and excluded from the quantitative metrics.
...
These values are sufficient for near real-time performance in some applications, since the total latency remains below 200 ms.%, which is often considered the upper limit for human-perceived real-time feedback.
...
The main advantage of the RPI devices lies in their low power consumption. The RPI5 consumed only 4.5 W during inference and less than 5 W in total, compared to 17.67 W for the laptop.
...
Overall, while short-term averages remained acceptable for walking speeds at lower frame rates ($\leq 22$ FPS), pushing the system above this envelope led to clear instability. To ensure safe real-time assistive navigation, the RPI5 on domestic 5G should therefore be operated conservatively below 20 FPS.
...
RPI5 power is currently an estimate based on CPU utilization scaling and should be interpreted qualitatively. Future work will include inline Direct Current(DC) measurements (INA219) to provide calibrated values and CIs.

This approach provides an approximate but practical measure of energy consumption without requiring external instrumentation, enabling lightweight evaluation of power trends during inference and streaming tasks.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{content/poweruse.png}
    \caption{Estimated power draw (W) by device/network, mean ± 95\% t-CI (proxy-based for RPI)}
    \label{fig:poweruse}
\end{figure}

Figure~\ref{fig:poweruse} compares the mean power consumption (poweruse\_W) across different benchmark setups, expressed as mean values with 95\% CIs. The results show a clear difference between laptops and RPI5 devices.  

The {laptop benchmarks} demonstrate significantly higher consumption, with an average of 15.6~W on the domestic Wi-Fi connection and 10.7 W when connected via 5G. The {RPI5}, in contrast, consumes only 3.5~W on the domestic network and 2.5 W over 5G. This makes the RPI4/5 far more suitable for battery-powered, mobile use cases, where long runtime is essential.  
...
An additional consideration is the economic feasibility of deployment. Assuming an average bitrate of 7 Mbit/s, while continuous streaming for one hour takes approximately 3.15 GB of mobile data use. With a domestic subscription of 15 GB/month at EUR 5 in Belgium (Hey! Telecom \cite{heytelecom_gsm}), this corresponds to nearly five hours of guided walking per month. This cost structure illustrates that the solution is not only technically viable but also affordable for end users.


\subsubsection{Conclusions on stability}
The field measurements confirm that public 5G NR-NSA networks provide sufficient stability for real-time assistive navigation in outdoor park environments. Median RTT of around 21 ms and low jitter (2–4 ms) indicate that end-to-end latency will remain well below the 200 ms threshold considered acceptable for haptic feedback and walking guidance.  
...
Overall, the observed throughput and RTT values are sufficient for real-time cloud-assisted segmentation, provided that the uplink bitrate remains below approximately 3–4 Mbps and local safety mechanisms (e.g., smart cane or onboard depth sensors) remain active.  
These findings confirm that the proposed system architecture can operate reliably over existing public 5G infrastructure, making large-scale deployment feasible without requiring private network slicing.
